...
const q = util.promisify(con.query).bind(con);
con.connect((err) => {
if (err) throw err;
console.log("success!")
...
// app.get('/',async(req,res)=>{
    //     for (let i = 0; i < clmname.length; i++) {
        ...
        //         res.render('combo',{clmname[i].combo_name});
        //     }
...
async function createDynamicSelectBox(clmname) {
    ...
    var sql = `SELECT option_name,option_id
FROM option_master
join select_master ON select_master.id=option_master.select_id where select_master.combo_name = '${comboname}'; `
...
app.get("/",async(req,res)=>{
    var dropdown_name =await q(`SELECT combo_name from combo.select_master;`);
    ...
    for(let i=0;i<dropdown_name.length;i++){
        ...
        clmname[dropdown_name[i].combo_name] = await createDynamicSelectBox(dropdown_name[i].combo_name)
    }
    res.render('combo',{clmname});
    })